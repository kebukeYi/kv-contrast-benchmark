...
func BenchmarkReadWithTrainDB(b *testing.B) {
	db := getTrainDB(b)
	defer db.Close()
...
func benchmarkReadWithTrainDB(b *testing.B, db *TrainDB.TrainKVDB, threshold int) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			key := randomKey(threshold)
			_, err := db.Get(key)
			if err != nil {
				atomic.AddInt32(&total, +1)
				// fmt.Printf("Error: %v, key:%s\n", err, key)
				// assert.Nil(b, err)
			}
		}
	})
	...
	reportRss(b)
	fmt.Printf("benchmarkReadWithTrainDB. total:%d\n", total)
}
...
func BenchmarkWriteWithTrainDB(b *testing.B) {
	db := getTrainDB(b)
	defer db.Close()
...
func benchmarkWriteWithTrainDB(b *testing.B, db *TrainDB.TrainKVDB) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			entry := &model.Entry{
				Key:   newKey(iteration, threshold),
				Value: value4KB,
			}
			entry.Key = model.KeyWithTs(entry.Key)
...
// -benchtime=60s -count=1 -timeout=30m
...
func BenchmarkDiskUsageWithTrainDB(b *testing.B) {
	db := getTrainDB(b)
	defer db.Close()
